  ...
  start: async (
    ...
    switch (inputCMD) {
      ...
      case "listbans": {
        ...
        try {
          ...
          if (!bannedUsers.length && !bannedGroups.length) {
            return sock.sendMessage(
              m.from,
              { text: `  🚫  *EXON XD Ban List*  🚫\n\nNo banned users or groups.` },
              { quoted: m },
            );
          }

          let banlistText = `  🚫  *EXON XD Ban List*  🚫\n\n`;
      ...
      case "characters": {
        ...
        let charListMenu = `  🎀  *EXON XD Characters*  🎀\n\n`;
        for (let i = 0; i < botNames.length; i++) {
          charListMenu += `  [ ${i} ] :  *${botNames[i]}*\n`;
        }
        charListMenu += `\n*Select a character ID and use* \`${prefix}setchar <ID>\` *to set it.*`;
        return sock.sendMessage(m.from, { text: charListMenu }, { quoted: m });
      }
